...
export function ottisSVG(state = 'idle') {
  const u = 'o' + (++seq);
  const BLACK = '#2B2725', BLACK2 = '#383331', WHITE = '#F7F3EB', SHADE = '#DCD4C6',
...
<svg class="ottis" data-state="${state}" viewBox="0 0 260 320"
  ...
  <defs>
    <pattern id="${u}-g" width="6" height="6" patternUnits="userSpaceOnUse">
  ...
  <g class="o-all">
    ...
    <g class="o-body">
      ...
      <path d="M130 150 C104 152 90 184 86 220 C82 256 84 292 92 304 L168 304
               C176 292 178 256 174 220 C170 184 156 152 130 150 Z" fill="${BLACK}"/>
      <path d="M130 166 C115 168 108 194 106 222 C104 252 108 284 114 300 L146 300
               C152 284 156 252 154 222 C152 194 145 168 130 166 Z" fill="${WHITE}"/>
      <path d="M110 248 C106 268 106 292 109 304 L125 304 C127 292 127 268 124 248 Z" fill="${WHITE}"/>
      <path d="M150 248 C154 268 154 292 151 304 L135 304 C133 292 133 268 136 248 Z" fill="${WHITE}"/>
      <path d="M127 250 C126 272 126 292 127 304 L133 304 C134 292 134 272 133 250 Z" fill="${SHADE}"/>
      <ellipse cx="117" cy="303" rx="9" ry="4.5" fill="${WHITE}"/>
      <ellipse cx="143" cy="303" rx="9" ry="4.5" fill="${WHITE}"/>
    </g>
    <path d="M106 126 C104 146 102 158 102 170 L158 170 C158 158 156 146 154 126 Z" fill="${BLACK}"/>
    <g clip-path="url(#${u}-b)"><rect x="95" y="162" width="72" height="50" fill="url(#${u}-g)"/></g>
    <path d="M101 168 C114 177 146 177 159 168 L145 203 C138 209 122 209 115 203 Z"
          fill="none" stroke="rgba(0,0,0,.22)" stroke-width="1.4"/>
    <g class="o-head">
      <g class="o-ear-l">
        <path d="M99 56 C78 46 62 68 65 96 C67 116 80 126 95 118 C89 100 92 72 99 56 Z" fill="${BLACK}"/>
        <path d="M95 62 C88 80 87 102 93 116" fill="none" stroke="${BLACK2}" stroke-width="3" stroke-linecap="round"/>
      </g>
      <g class="o-ear-r">
        <path d="M161 56 C182 46 198 68 195 96 C193 116 180 126 165 118 C171 100 168 72 161 56 Z" fill="${BLACK}"/>
      ...
      <path d="M130 44 C158 44 175 62 175 90 C175 112 162 128 148 134
               L112 134 C98 128 85 112 85 90 C85 62 102 44 130 44 Z" fill="${BLACK}"/>
      ...
      <path d="M130 98 C147 98 155 112 154 128 C153 143 143 152 130 152
               C117 152 107 143 106 128 C105 112 113 98 130 98 Z" fill="${BLACK}"/>
      ...
      <g class="o-lid-l"><ellipse cx="108" cy="90" rx="12" ry="12" fill="${BLACK}"/></g>
      <g class="o-lid-r"><ellipse cx="152" cy="90" rx="12" ry="12" fill="${BLACK}"/></g>
